...
* The logistic regression uses 'snapshot' data: whether the observation has died at a given point in time. Each independant variable can increase or decrease the probability of being dead at that point.

* Survival analysis is interested in analyzing and modelizing the survivor function of the dataset over time and takes in account the survival time observed.
...
d <- data.frame(time = fit$time,
                  ...
                  n.event = fit$n.event,
                  n.censor = fit$n.censor,
                  int.risk = fit$n.event/fit$n.risk,
                  int.surv = 1-(fit$n.event/fit$n.risk)
                  )
head(d, 10)
```
...
* Look at the p-value for the overall association and the 3rd column for the association by level. The more distant from 1, the more unexpected this count of events is.
...
* Visually, curves should be proportional and can not cross. ! Still needs proper testing !
...
**Those coefficients are not interpretable, but if they were...**
...
* Survival analysis is a powerful tool for analyzing the association of variables with a hazard function (a therefore a probability of survival)
...
* The proportional hazards assumption is a strong one, and hard to meet,especially true for large datasets
...
* [CRAN, *Plotting with survival package*](https://cran.r-project.org/web/packages/ggfortify/vignettes/plot_surv.html)
